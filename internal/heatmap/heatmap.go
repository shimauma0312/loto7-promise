...
// 最も出現回数の多い数字を見つける（上位3グループまで）
func findHottestNumbers(overallCounts map[int]int) []int {
	// 出現回数でグループ化
	countGroups := make(map[int][]int)
	maxCount := 0

	for number := 1; number <= 37; number++ {
		count := overallCounts[number]
		if count > maxCount {
			maxCount = count
		}

		if countGroups[count] == nil {
			countGroups[count] = []int{}
		}
		countGroups[count] = append(countGroups[count], number)
	}
	...
	for groupCount < 3 && count >= 0 {
		if numbers, exists := countGroups[count]; exists {
			hottest = append(hottest, numbers...)
			groupCount++
		}
		count--
	}
...
func findColdestNumbers(overallCounts map[int]int) []int {
	...
	for groupCount < 3 && count <= 999999 {
		if numbers, exists := countGroups[count]; exists {
			coldest = append(coldest, numbers...)
			groupCount++
		}
		count++
	}
